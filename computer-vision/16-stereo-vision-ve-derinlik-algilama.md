...
Bu farka **Disparity (Eşitsizlik)** denir.
...
En basit stereo sistemde, iki kamera birbirine paralel, optik eksenleri aynı yöne bakacak ve görüntü düzlemleri aynı hizada olacak şekilde yerleştirilir. Bu idealize edilmiş durumda, bir `P` noktasının derinliği (`Z`) ile `disparity`'si (`d`) arasında ters orantılı bir ilişki vardır:
...
*Görsel: Paralel bir stereo kamera sisteminde, P noktasının sol ve sağ görüntülerdeki izdüşümleri arasındaki fark olan `disparity` (d), noktanın derinliği (Z) ile ters orantılıdır.*
...
## 4. The Correspondence Problem (Eşleştirme Problemi)
...
Bu eşleştirmeyi yapmak için genellikle, sol görüntüdeki küçük bir pencere (örneğin 5x5 piksel) alınır ve sağ görüntüdeki `epipolar` çizgi üzerinde kaydırılarak, bu pencereye en çok benzeyen bölge aranır. Benzerlik, `SSD (Sum of Squared Differences)` veya `Normalized Cross-Correlation (NCC)` gibi metriklerle ölçülür.
...
*   **Occlusion (Tıkanma):** Bir gözün gördüğü bir nokta, diğeri tarafından görülemeyebilir.
...
<div class="quiz-question">
  <p><b>Soru 1:</b> Stereo vision sisteminde, bir nesne kameralara ne kadar yakınsa, `disparity`'si o kadar ... olur.</p>
...
<div class="quiz-question">
  <p><b>Soru 2:</b> `Epipolar constraint`'in (epipolar kısıtlama) stereo eşleştirme problemine sağladığı en büyük avantaj nedir?</p>
