---
layout: default
title: Scale-Invariant Features ve SIFT Detector
nav_order: 18
parent: Computer Vision
---

# Scale-Invariant Features ve SIFT Detector

Harris köşe dedektörü, döndürme gibi dönüşümlere karşı dayanıklı olsa da, en büyük zayıflığı ölçek değişikliklerine karşı hassas olmasıydı. Bir nesneye yaklaştıkça veya uzaklaştıkça, Harris dedektörü aynı noktaları güvenilir bir şekilde bulamaz. **SIFT (Scale-Invariant Feature Transform)**, tam olarak bu sorunu çözmek için geliştirilmiştir.

SIFT, sadece bir köşe dedektörü değil, aynı zamanda bu noktaları tanımlayan bir "tanımlayıcı" (descriptor) da üreten komple bir sistemdir. Bu bölümde, SIFT'in ilk adımı olan **ölçek-dayanıklı anahtar nokta tespitini (scale-invariant keypoint detection)** inceleyeceğiz.
...
1.  Orijinal görüntünün farklı seviyelerde bulanıklaştırılmış (smoothed) ve yeniden boyutlandırılmış versiyonlarından oluşan bir **ölçek uzayı piramidi (scale-space pyramid)** oluşturulur.
...
*Görsel: Görüntünün farklı boyutlarda (oktavlar) ve her boyutta farklı seviyelerde Gaussian filtresiyle bulanıklaştırılmasıyla bir ölçek uzayı oluşturulur.*

---

## 2. Karakteristik Ölçeği Bulmak: DoG (Difference of Gaussians)
...
## 3. Anahtar Nokta Tespiti (Keypoint Localization)

Bir noktanın "anahtar nokta" (keypoint) olarak kabul edilmesi için, sadece kendi 2D komşularına göre değil, aynı zamanda ölçek olarak bir alt ve bir üst seviyedeki komşularına göre de bir tepe noktası (maksimum veya minimum) olması gerekir.
...
<div class="quiz-question">
  <p><b>Soru 1:</b> SIFT algoritmasının ölçek dayanıklılığını (scale invariance) sağlamak için kullandığı temel yapı nedir?</p>
...
<div class="quiz-question">
  ...
  <div class="quiz-option" data-correct="true">C) DoG (Difference of Gaussians)</div>
