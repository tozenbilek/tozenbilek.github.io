...
## 1. Segmentation as a Clustering Problem
...
*   **Color (RGB - Renk):** Her piksel, 3 boyutlu bir `(R, G, B)` uzayında bir noktadır.
*   **Intensity (Parlaklık):** Gri tonlamalı bir görüntü için, her piksel 1 boyutlu bir uzayda bir noktadır.
*   **Position (Konum):** Piksellerin sadece renkleri değil, aynı zamanda görüntüdeki `(x, y)` koordinatları da özellik olarak eklenebilir. Bu, birbirine yakın ve benzer renkteki piksellerin aynı grupta olmasını teşvik eder.
*   **Texture (Doku):** Daha ileri seviyede, her pikselin etrafındaki bölgenin dokusunu (pürüzlü, çizgili vb.) temsil eden bir özellik vektörü de kullanılabilir.
...
2.  **Assignment (Atama):** Her bir pikseli, kendisine en yakın olan küme merkezine ata. Bu, özellik uzayını `k` adet bölgeye ayırır.
3.  **Update (Güncelleme):** Her bir küme merkezini, o kümeye atanmış olan tüm piksellerin ortalama konumuna (veya rengine) taşı.
...
<div class="quiz-question">
  <p><b>Soru 1:</b> `Image segmentation`'ı bir `clustering` (kümeleme) problemi olarak ele aldığımızda, en basit durumda kümelenen "şey" nedir?</p>
