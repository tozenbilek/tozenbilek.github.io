...
*   **Kalıcı Depolama:** Güç kesildiğinde bile veriyi saklar.
    ...
    *   **Manyetik Disk (HDD):** Dönen plakalar ve okuma/yazma kafaları kullanan, daha yavaş ama daha ucuz ve yüksek kapasiteli geleneksel sürücüler.
...
**Temel Fikir:** Her katman, bir altındaki daha yavaş ve daha büyük katman için bir **önbellek (cache)** görevi görür. En sık kullanılan veriler, piramidin en tepesine yakın, en hızlı katmanlarda tutulmaya çalışılır.

---

## 3. Yerellik Prensibi (Principle of Locality)
...
### a) Zamansal Yerellik (Temporal Locality)
...
### b) Uzamsal Yerellik (Spatial Locality)
...
## 4. Önbellekler (Caches) Nasıl Çalışır?

Önbellek, ana bellekten okunan verileri geçici olarak saklayan küçük ve hızlı bir SRAM belleğidir.
*   Veri, **blok (block)** adı verilen sabit boyutlu parçalar halinde önbelleğe kopyalanır.
*   İşlemci bir veriye ihtiyaç duyduğunda önce önbelleğe bakar.
    *   **Önbellek Vuruşu (Cache Hit):** Eğer veri önbellekte varsa, çok hızlı bir şekilde alınır.
    *   **Önbellek Iskalama (Cache Miss):** Eğer veri önbellekte yoksa, işlemci duraksar. Verinin bulunduğu tüm blok, ana bellekten önbelleğe kopyalanır ve sonra işlemciye sunulur. Bu işlem, bir "hit"e göre çok daha yavaştır (**miss penalty**).
...
## 5. Önbellek Dostu (Cache-Friendly) Kod Yazmak
...
```
C dilinde matrisler bellekte **satır-öncelikli (row-major)** olarak saklandığı için, `sum_row_major` fonksiyonu belleğe sıralı erişim yapar. Bu, mükemmel bir uzamsal yerellik örneğidir. Bir eleman için yaşanan "miss", tüm bir satırı önbelleğe getireceği için o satırdaki diğer elemanlara erişim "hit" olacaktır.
...
<div class="quiz-question">
  ...
  <div class="quiz-option" data-correct="true">C) Yazmaçlar (Registers) - En yüksek hız</div>
...
<div class="quiz-question">
  ...
  <div class="quiz-option" data-correct="true">A) Zamansal Yerellik (Temporal Locality)</div>
  <div class="quiz-option">B) Uzamsal Yerellik (Spatial Locality)</div>
  <div class="quiz-option">C) Sıralı Yerellik (Sequential Locality)</div>
  <div class="quiz-option">D) Rastgele Yerellik (Random Locality)</div>
  <div class="quiz-explanation">
...
<div class="quiz-question">
  ...
  <div class="quiz-explanation">
    <p><b>Cevap: C.</b> Yüksek bir "cache miss rate", programın yerellik prensibinden iyi faydalanamadığını gösterir. Bellek erişimlerini daha sıralı ve öngörülebilir hale getirmek (uzamsal yerelliği artırmak), önbellek vuruş oranını (hit rate) artıracak ve performansı önemli ölçüde iyileştirecektir. Diğer seçenekler yardımcı olsa da, temel sorunu çözmezler.</p>
  </div>
</div>
